...
std::vector<Edge> primElogE(std::vector<std::vector<Node>> graph) {
	...
	std::vector<bool> included(graph.size(), false); // lista para saber quais nós estão na árvore, iniciada como false, pois ainda não tem nenhum nó add
	std::priority_queue<Edge, std::vector<Edge>, GreaterEdge> edges({}); // fila de prioridade: tipo, estrutura de dados, 
																			//critério de prioridade (GreaterEdge - maior prioridade os elementos de menor valor
	std::vector<Edge> tree; // conforme tira da fila de prioridade, add nessa arvore, que será retornada no final

	edges.push(Edge(-1, 0, 0)); // pega da raiz (-1, nao existe), para o nó zero com custo zero.

	while (!edges.empty()) { // enquanto a fila não estiver vazia
